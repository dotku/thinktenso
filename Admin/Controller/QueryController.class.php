...
class QueryController extends Controller {
    ...
    public function stock() {
        $table = M('pkg');
        $this->table = $table->order('create_time desc')->limit(1)->where('ifRecycle = 0')->select();
    ...
    public function recycle() {
        $table = M('pkg');
        if (!I('path.2')) {
            $this->table = $table->where('ifRecycle = 1')->select();
            $this->display('Tenso/recycle');
        } else {
            $data['ifRecycle'] = 1;
            $table->where('pkg_id='.I('path.2'))->save($data);
			$sResult['msg'] = '物品 '.I('path.2').' 已经丢入回收站"';
