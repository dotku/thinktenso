			...
			<form method="post">
				<table class="table table-bordered">
					<tr>
						...
						<td>
								<select name="pkg_id">
									<?php if(is_array($tablePkg)): $i = 0; $__LIST__ = $tablePkg;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["pkg_id"]); ?>">入库编号: <?php echo ($vo["pkg_id"]); ?> | 来源编号: <?php echo ($vo["pkg_orig_number"]); ?> | 客户编号: <?php echo ($vo["user_id"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
					...
					<tr>
						<th>发货编号
						<td><input type="text" name="pkg_ship_number" class="input-xxlarge" placeholder="请输入发货编号"></input></td>
					</tr>
				</table>
				<input type="hidden" name="pkg_status" value="confirm"/>
